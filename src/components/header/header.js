export function initHeader() {
  ...
  function highlightActiveMenuItem() {
    ...
    // Удаляем все существующие классы --active
    const allActiveLinks = header?.querySelectorAll(
      '.header__nav-link--active, .header__dropdown-link--active'
    );
    allActiveLinks?.forEach((link) => {
      link.classList.remove('header__nav-link--active', 'header__dropdown-link--active');
    });

    // Находим и подсвечиваем соответствующий пункт меню
    switch (currentPage) {
      case 'home': {
        const homeLink = header?.querySelector('.header__nav-link[href="/"]');
      ...
      case 'about': {
        const aboutLink = header?.querySelector(
          '.header__nav-link[href="/pages/about/index.html"]'
        );
        aboutLink?.classList.add('header__nav-link--active');
        break;
      }

      case 'blog': {
        const blogLink = header?.querySelector('.header__nav-link[href="/pages/blog/index.html"]');
        blogLink?.classList.add('header__nav-link--active');
        break;
      }

      case 'price': {
        const priceLink = header?.querySelector(
          '.header__nav-link[href="/pages/price/index.html"]'
        );
        priceLink?.classList.add('header__nav-link--active');
        break;
      }

      case 'residential': {
        const residentialLink = header?.querySelector(
          '.header__dropdown-link[href="/pages/residential/index.html"]'
        );
        residentialLink?.classList.add('header__dropdown-link--active');

        // Подсвечиваем также пункт меню Services
        const servicesLink = header?.querySelector('.header__nav-link--dropdown-trigger');
        servicesLink?.classList.add('header__nav-link--active');
        break;
      }

      case 'automotive': {
        const automotiveLink = header?.querySelector(
          '.header__dropdown-link[href="/pages/automotive/index.html"]'
        );
        automotiveLink?.classList.add('header__dropdown-link--active');
      ...
      case 'commercial': {
        const commercialLink = header?.querySelector(
          '.header__dropdown-link[href="/pages/commercial/index.html"]'
        );
        commercialLink?.classList.add('header__dropdown-link--active');
      ...
      case 'cleaning': {
        const cleaningLink = header?.querySelector(
          '.header__dropdown-link[href="/pages/cleaning/index.html"]'
        );
        cleaningLink?.classList.add('header__dropdown-link--active');
  ...
  function getCurrentPage() {
    ...
    // Страницы Services
    if (path.includes('/pages/residential/')) {
      return 'residential';
    }
    if (path.includes('/pages/automotive/')) {
      return 'automotive';
    }
    if (path.includes('/pages/commercial/')) {
      return 'commercial';
    }
    if (path.includes('/pages/cleaning/')) {
      return 'cleaning';
    }
    ...
    // Страницы Blog (включая подстраницы)
    if (path.includes('/pages/blog/')) {
      return 'blog';
    }

    // Страница Price
    if (path.includes('/pages/price/')) {
      return 'price';
    }
